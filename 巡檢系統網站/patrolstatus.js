...
window.PatrolStatus = (function () {
  ...
  // 若距下一個班別開始在 1 小時以內，則顯示該尚未開始的班別（待打卡紫色）。
  async function compute(db, dateStr) {
    ...
    // 2) 沒有進行中 → 找「最近已結束」班別，但若距下一個班別開始 ≤ 1 小時，
    //    則改為顯示下一個班別（顏色 = 待打卡紫色，非逾期紅色）
    if (!relevant) {
      // 找最近已結束的班別
      let bestEnd = null;
      for (const s of allShifts) {
        const r = shiftRange(s, s._base);
        if (r.end <= now && (!bestEnd || r.end > bestEnd)) {
          relevant = s; relevantRange = r; bestEnd = r.end;
        }
      }

      // 找下一個即將開始的班別
      let nextShift = null, nextRange = null;
      for (const s of allShifts) {
        const r = shiftRange(s, s._base);
        if (r.start > now) {
          if (!nextShift || r.start < nextRange.start) {
            nextShift = s; nextRange = r;
          }
        }
      }

      // 若距下一個班別 ≤ 1 小時，改用該班別
      if (nextShift && nextRange) {
        const diff = nextRange.start.getTime() - now.getTime();
        if (diff <= 60 * 60 * 1000) {
          relevant = nextShift;
          relevantRange = nextRange;
        }
      }
    }
