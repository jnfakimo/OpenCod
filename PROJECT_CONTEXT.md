...
- **全站表頭功能按鈕由共用元件管理**：`system/theme.js` 固定建立「戰情儀表板」→「報修系統」→「完工回報」→「後台」四個按鈕，並使用戰情儀表板既有的 `ic-*` 圖示系統。圖示只能引用既有檔案 `system/icons/nav-dashboard.png` 與 `system/icons/nav-dispatch.png`，不得重新生成、重繪或替換。各頁不得再以 emoji、文字符號或大型彩色 PNG 自行建立這四個按鈕；頁面專用功能按鈕可保留在共用按鈕左側。此四個 ICON 的樣式、順序及共用元件實作列為鎖定規範，未經使用者明確要求變更此標準，不得再更換。
...
- 圖示統一採用戰情儀表板既有的 `ic-*` 系統；移除各頁原有的 emoji、文字符號與大型彩色 PNG 差異。
- 直接引用既有 `system/icons/nav-dashboard.png` 與 `system/icons/nav-dispatch.png`，本次未重新生成或修改任何 ICON 圖檔；後續亦不得重新生成、重繪或替換。
...
- 已以瀏覽器驗證 `repair.html`、`equipment.html` 與代表性表頭：四個按鈕順序、圖示、登入者、連線狀態及台北時間排列正確。
