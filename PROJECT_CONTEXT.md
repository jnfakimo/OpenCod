...
- **全站表頭功能按鈕由共用元件管理**：`system/theme.js` 固定建立「戰情儀表板」→「報修系統」→「完工回報」→「後台」四個按鈕，並使用戰情儀表板既有的 `ic-*` 圖示系統。各頁不得再以 emoji、文字符號或大型彩色 PNG 自行建立這四個按鈕；頁面專用功能按鈕可保留在共用按鈕左側。此四個 ICON 的樣式、順序及共用元件實作列為鎖定規範，未經使用者明確要求變更此標準，不得再更換。
