...
- **Shared header actions**: `system/theme.js` owns the global action group. Every
  ...
  Reuse the existing dashboard assets `system/icons/nav-dashboard.png` and
  `system/icons/nav-dispatch.png`; never regenerate, redraw, or replace them for
  this shared header without an explicit user request.
